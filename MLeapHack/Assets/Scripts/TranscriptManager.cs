...
namespace MagicLeap
{
    public class TranscriptManager : MonoBehaviour
    {
        ...
        public static TranscriptManager _instance { get; set; }
        public MediaPlayerExample mpe;
        string originalText;
        ...
        void Start()
        {
            ToggleAdditionalInfoPanel(false);
        ...
        void Update()
        {
            ...
            {
                if ((mpe.GetElapsedTime() / 1000) > transcriptSentenceList[currentTimeStampIndex].timeStamp)
                {
        ...
        public void FastForwardTranscript()
        {
            int newIndex = currentTimeStampIndex;

            if (transcriptSentenceList == null)
            {
                return;
            }

            while ((mpe.GetElapsedTime() / 1000) > transcriptSentenceList[newIndex].timeStamp)
            {
                newIndex++;
            }
            for (int i = currentTimeStampIndex; i <= newIndex; i++)
            {
                transcriptText.text = transcriptText.text.Replace(transcriptSentenceList[i].originalSentence, transcriptSentenceList[i].newSentence);
            }
            currentTimeStampIndex = newIndex;
        }
        
        public void BacktrackTranscript()
        {
            ResetTranscript();
            FastForwardTranscript();
        }
        ...
        public void ToggleAdditionalInfoPanel(bool condition)
        {
            additionalInfoPanel.SetActive(condition);
        }
